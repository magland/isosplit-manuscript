...
\begin{figure}
\begin{center}
%%simulation2.eps copied from arxiv submission -- selected example by hand
...
\begin{figure}
\begin{center}
%%generated from scripts/FIG04_example_dbscan.m
\includegraphics[width=5.5in]{images/example_dbscan.eps}
\end{center}
\caption{
...
\begin{figure}
\begin{center}
%%generated by scripts/FIG05_nongaussian_histogram.jpg
...
\begin{figure}
\begin{center}
%copied from arxiv submission -- selected example by hand
...
\begin{figure}
\begin{center}
%% Generated from scripts/FIG_simulation_results_graph.m which I believe parses this document!
...
The algorithms considered in this paper were applied to spike sorting of neural electrophysiological signals, that is, the clustering of spiking events in a time series into $K$ clusters that can often be associated with individual neurons. Clustering is a key step in spike sorting.

[Boilerplate text for describing dataset: dl12\_20151208\_NNF\_r1\_tet16\_17]

After standard preprocessing, including bandpass filter and channel whitening, spiking events were detected at timepoints where the signal exceeded a fixed threshold (around 5 standard deviations above the mean). Time windows (or clips) of 3.3 milliseconds (100 timepoints) were extracted around each identified timepoint. These clips were mapped into PCA space and ISO-SPLIT clustering was applied. 

The resulting clustering (neuron labelings) along with the centroid waveforms are shown in Figure \ref{fig:real_data_1}. Various validation methods, and human judgements, are now commonly applied to determine whether each cluster is a single neural unit, e.g., \cite{Hill2011}; we do not attempt to perform that here. We emphasize that ISO-SPLIT required no adjustment of free parameters.

\begin{figure}
\begin{center}
\includegraphics[width=5.5in]{images/real_data.eps}
\end{center}
\caption{
Needed
